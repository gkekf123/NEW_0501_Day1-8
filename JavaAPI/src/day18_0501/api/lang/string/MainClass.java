...
//		String str = new String(arr);
		...
		str.indexOf("안녕");
		System.out.println(str.indexOf("안녕"));	// 찾으면 인덱스 번호 반환
		System.out.println(str.indexOf("xx"));	// 못찾으면 -1 반환
		
		if(str.indexOf("안녕") != -1) {
			System.out.println("문자에 값이 존재 한다");
		}
		...
		// 문자열의 비교
		str = "홍길동";
		System.out.println(str.equals("홍길동"));		// 문자열이 같으면 true, 다르면 false
		
		// 사전 등재순의 비교
		System.out.println(str.compareTo("홍길동"));	// 0 - 같은문자
		System.out.println(str.compareTo("홍길자"));	// 음수 - str이 홍길자 보다 사전등재순 앞에 위치 함
		System.out.println(str.compareTo("홍길가"));	// 양수 - str이 홍길가 보다 사전둥재순 뒤에 위치 함
		
		// 특정 구분자로 문자열을 연결
		System.out.println(String.join("-", "hello", "world", "안녕", "하세요"));
	
		// 기본타입을 문자열로 변경
		System.out.println(String.valueOf(1) + String.valueOf(2));	// 위아래 같다
		System.out.println("" + 1 + 2);	// 위아래 같다
